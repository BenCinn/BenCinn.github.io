...
export default function Blog(props) {
  if (props.latest == false){
    ...
    return (
      <Link href={`blog/${props.slug}`}>
      <a><div className="overflow-hidden max-w-sm rounded shadow-lg transition duration-300 hover:shadow-xl">
      <img className="w-full" src={props.img} alt={props.text} />
      <div className="px-6 py-4">
      <div className="mb-2 text-xl font-bold">{props.text}</div>
      <p className="text-base text-gray-700">
      ...
      <div className="px-6 pt-4 pb-3">
        ...
        <span className="inline-block px-3 py-1 mr-2 mb-2 text-sm font-semibold text-gray-700 bg-yellow-200 rounded-full">newest</span>
      </div>
    </div></a></Link>
  );
  }
